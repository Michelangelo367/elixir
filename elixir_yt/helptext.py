...
def helptext():
    ...
    Access songs that have been played within elixir
    ...
    All elixir commands can be entered from the command line.  For example;

      {2}elixir dlurl <url or id>{1} to download a YouTube video by url or id
      {2}elixir playurl <url or id>{1} to play a YouTube video by url or id
      {2}elixir /mozart{1} to search
      {2}elixir //best songs of 2010{1} for a playlist search
      {2}elixir play <playlist name or ID>{1} to play a saved playlist
      {2}elixir ls{1} to list saved playlists

    For further automation, a series of commands can be entered separated by
    commas (,).  E.g.,

      {2}elixir open 1, 2-4{1} - play items 2-4 of first saved playlist
      {2}elixir //the doors, 1, all -a{1} - open YouTube playlist and play audio
...
def get_help(choice):
    ...
    helps = {"download": ("playback dl listen watch show repeat playing"
             ...
             "invoke": "command commands elixir invocation".split(),
