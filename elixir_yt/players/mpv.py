...
class mpv(CmdPlayer):
    ...
    def launch_player(self, cmd):
        ...
        if self.mpv_usesock:
            self.sockpath = tempfile.mktemp('.sock', 'elixir-mpv')
            ...
            if g.mprisctl:
                self.fifopath = tempfile.mktemp('.fifo', 'elixir-mpv')
...
def _get_input_file():
    ...
    Return file name of temp input file with elixir mappings included
    """
    confpath = conf = ''
    ...
    with tempfile.NamedTemporaryFile('w', prefix='elixir-input',
