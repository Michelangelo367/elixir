...
def _do_query(url, query, err='query failed', report=False):
    """ Perform http request using elixir user agent header.
    ...
    elixir_opener = build_opener()
    elixir_opener.addheaders = [('User-agent', ua)]
    ...
    try:
        wdata = elixir_opener.open(url).read().decode()
